...
module.exports = {
  loadCart: async (req, res) => {
    try {
      ...
      if (user_id) {
        ...
        const subtotal = await cartData?.product?.reduce(
      ...
      if (!user_id) {
        ...
        if (productData.quantity > 0) {
          ...
          if (cartProduct) {
            return res
              .status(200)
              .json({ success: false, error: "product alredy in cart" });
          }
  ...
  updateCart: async (req, res) => {
    try {
      ...
      if (count === -1) {
        const currentQuantity = cartD.product.find((p) =>
      ...
      if (count === 1) {
        ...
        if (currentQuantity + count > product.quantity) {
          return res.json({
            success: false,
            message: "Cannot add more than the quantity.",
          });
        }
      }
    ...
    try {
      ...
      if (cartData) {
        ...
        const discountPercentage =
          cartData.couponDiscount != 0
            ? cartData.couponDiscount.discountPercentage
            : 0;
        const maxDiscount =
          cartData.couponDiscount != 0
            ? cartData.couponDiscount.maxDiscountAmount
            : 0;
        const subtotal = await cartData?.product?.reduce(
